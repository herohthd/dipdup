...
exports.validateConfigs = void 0;
const chalk_1 = __importDefault(require("chalk"));
const validateConfigs = (configs) => {
    const errors = [];
    if (configs.length === 0)
        errors.push('config array is empty');
    configs.forEach((config, i) => {
        ...
        if ([config.endpoint, config.schema].filter((i) => i).length > 1)
            errors.push(`you provided two or more conflicting options in ${whichConfig}, only one of either \`endpoint\`, \`schema\` or \`fetcher\` is allowed`);
        if (!config.output)
            errors.push(`you didn't provide an \`output\` option in ${whichConfig}`);
    });
    errors.forEach((error) => console.log(chalk_1.default.red(`Error: ${error}`)));
