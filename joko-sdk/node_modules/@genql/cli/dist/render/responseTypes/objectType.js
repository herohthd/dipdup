...
const objectType = (type, ctx) => {
    ...
    if ((_a = ctx.config) === null || _a === void 0 ? void 0 : _a.sortProperties) {
        fieldsMap = support_1.sortKeys(fieldsMap);
    }
    const fields = Object.keys(fieldsMap).map((fieldName) => fieldsMap[fieldName]);
    if (!ctx.schema)
        throw new Error('no schema provided');
    const typeNames = graphql_1.isObjectType(type)
        ? [type.name]
        : ctx.schema.getPossibleTypes(type).map((t) => t.name);
    let fieldStrings = fields
        .map((f) => {
        return `${comment_1.fieldComment(f)}${f.name}${renderTyping_1.renderTyping(f.type, true, true)}`;
    })
        .concat([
    ...
    ctx.addCodeBlock(`${comment_1.typeComment(type)}export interface ${type.name} {\n${fieldStrings.join('\n')}\n}`);
};
